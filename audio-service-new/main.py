...
def text_to_speech(text) -> str | None:
    ...
    # First convert to katakana (Japanese phonetic representation of English)
...
def heartbeat():
    """Heartbeat every 5 seconds (silent)"""
    while not shutdown_flag.wait(5):
        # Silent heartbeat - no stdout output for IPC
        pass
...
def main_loop():
    ...
    while not shutdown_flag.is_set():
        try:
            ...
            base_path = text_to_speech(user_input)
            if base_path == None:
                sys.stderr.write("Error: Could not generate base vocals\n")
                continue
            
            dubbed_path = convert_audio(base_path)
            if dubbed_path == None:
                sys.stderr.write("Error: Could not generate dubbed vocals\n")
                continue
                
            play_audio(dubbed_path)
            
            # Clean up all wav files in tmp directory
            cleanup_tmp_files()
        except EOFError:
            break
